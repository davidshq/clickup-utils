...
fn extract_task_id(output: &str) -> Option<String> {
    ...
    let uuid_pattern = regex::Regex::new(r"[a-f0-9]{8}-[a-f0-9]{4}-[a-f0-9]{4}-[a-f0-9]{4}-[a-f0-9]{12}").unwrap();
...
fn get_or_discover_workspace_id() -> Option<String> {
    ...
    for line in stdout.lines() {
        if line.trim().starts_with("|") && line.contains("|") && line.chars().filter(|&c| c == '|').count() > 2 {
...
fn test_authentication() {
    ...
    assert!(output_str.contains("ID") || output_str.contains("Name"), 
...
fn test_task_lifecycle() {
    ...
    let task_id = match extract_task_id(&output_str) {
        ...
        None => {
            ...
            if created { delete_list(&list_id); }
            return;
        }
    };
    ...
    let output = cmd.output().unwrap();
    assert!(output.status.success(), "Task show failed: {:?}", String::from_utf8_lossy(&output.stderr));
    ...
    let output = cmd.output().unwrap();
    assert!(output.status.success(), "Task update failed: {:?}", String::from_utf8_lossy(&output.stderr));
    ...
    let output = cmd.output().unwrap();
    assert!(output.status.success(), "Task show after update failed: {:?}", String::from_utf8_lossy(&output.stderr));
    ...
    let output = cmd.output().unwrap();
    assert!(output.status.success(), "Task deletion failed: {:?}", String::from_utf8_lossy(&output.stderr));
    ...
    let output = cmd.output().unwrap();
    assert!(!output.status.success(), "Task should be deleted and show should fail");
...
fn test_commenting() {
    ...
    let comment_text = format!("Integration test comment {}", chrono::Utc::now().timestamp());
    let updated_comment_text = format!("Updated integration test comment {}", chrono::Utc::now().timestamp());
    ...
    let output = cmd.output().unwrap();
    assert!(output.status.success(), "Comment creation failed: {:?}", String::from_utf8_lossy(&output.stderr));
    ...
    let output = cmd.output().unwrap();
    assert!(output.status.success(), "Comment listing failed: {:?}", String::from_utf8_lossy(&output.stderr));
    
    let output_str = String::from_utf8_lossy(&output.stdout);
    assert!(output_str.contains(&comment_text), "Comment text not found in listing");
    ...
    let output = cmd.output().unwrap();
    assert!(output.status.success(), "Comment update failed: {:?}", String::from_utf8_lossy(&output.stderr));
    ...
    let output = cmd.output().unwrap();
    assert!(output.status.success(), "Comment listing after update failed: {:?}", String::from_utf8_lossy(&output.stderr));
    
    let output_str = String::from_utf8_lossy(&output.stdout);
    assert!(output_str.contains(&updated_comment_text), "Updated comment text not found in listing");
    ...
    let output = cmd.output().unwrap();
    assert!(output.status.success(), "Comment deletion failed: {:?}", String::from_utf8_lossy(&output.stderr));
    ...
    let output = cmd.output().unwrap();
    assert!(output.status.success(), "Task cleanup failed: {:?}", String::from_utf8_lossy(&output.stderr));
...
fn test_error_handling() {
    ...
    // Should fail with an error
    assert!(!output.status.success(), "Expected failure for non-existent task in comment creation");
    assert!(output.stderr.len() > 0, "Should have error output");
    ...
    // Should fail with an error
    assert!(!output.status.success(), "Expected failure for non-existent comment");
    assert!(output.stderr.len() > 0, "Should have error output");
}
...
fn test_rate_limiting() {
    ...
    for _i in 0..10 {
        ...
        if output.status.success() {
            ...
            if stderr.contains("429") || stderr.contains("rate limit") || stderr.contains("too many requests") {
                rate_limit_count += 1;
            }
        }
    ...
    assert!(success_count > 0, "No requests succeeded, possible authentication issue");
    
    // If we hit rate limits, that's expected behavior
    if rate_limit_count > 0 {
        println!("Rate limiting detected in {} out of 10 requests", rate_limit_count);
    }
}
...
fn test_list_operations() {
    ...
    let output = cmd.output().unwrap();
    assert!(output.status.success(), "Space listing failed: {:?}", String::from_utf8_lossy(&output.stderr));
    ...
    if let Some(list_id) = get_test_list_id() {
        ...
        let output = cmd.output().unwrap();
        assert!(output.status.success(), "Task listing failed: {:?}", String::from_utf8_lossy(&output.stderr));
    }
}
...
fn test_invalid_authentication() {
    ...
    // Should fail with an error
    assert!(!output.status.success(), "Expected failure with invalid token");
    assert!(output.stderr.len() > 0, "Should have error output");
