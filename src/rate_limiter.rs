        ...
        Box::pin(async move {
            ...
            loop {
                ...
                if current_requests >= self.config.requests_per_minute {
                    ...
                    info!("Rate limit reached ({} requests in last minute). Waiting {} seconds...", 
                          current_requests, actual_wait.as_secs());
                ...
                debug!("Request allowed. {} requests in last minute.", current_requests + 1);
                return Ok(());
            }
        })
    }
    ...
    pub async fn handle_rate_limit(&self, retry_after_seconds: Option<u64>) -> Result<(), ClickUpError> {
        ...
        *retry_count += 1;
        ...
        if *retry_count > self.config.max_retries {
            warn!("Exceeded maximum retry attempts ({}) for rate-limited request", self.config.max_retries);
        ...
        info!("Rate limited. Retry attempt {}/{}. Waiting {} seconds...", 
              *retry_count, self.config.max_retries, wait_time.as_secs());
