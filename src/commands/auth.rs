...
pub async fn execute(command: AuthCommands, config: &mut Config) -> Result<(), ClickUpError> {
    match command {
        ...
        AuthCommands::RateLimit { requests_per_minute, auto_retry, max_retries, buffer_seconds, show } => {
            configure_rate_limit(config, requests_per_minute, auto_retry, max_retries, buffer_seconds, show)?;
        }
    }
    Ok(())
}
...
fn configure_rate_limit(
    ...
    if show {
        ...
        println!("Requests per minute: {}", config.rate_limit.requests_per_minute);
    ...
    if let Some(rpm) = requests_per_minute {
        if rpm == 0 {
            return Err(ClickUpError::ValidationError("Requests per minute must be greater than 0".to_string()));
        }
    ...
    if let Some(max) = max_retries {
        if max == 0 {
            return Err(ClickUpError::ValidationError("Max retries must be greater than 0".to_string()));
        }
    ...
    if updated {
        config.save()?;
        println!("{}", "✓ Rate limiting configuration saved successfully!".green());
    } else {
        println!("{}", "No changes made. Use --show to see current configuration.".yellow());
    }
